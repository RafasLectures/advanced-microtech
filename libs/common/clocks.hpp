 ...
 * @date    25.12.2014
 *
 * @brief   Header file that contains abstractions for the clock. Taken from:
 *          https://github.com/ekoeppen/msp430-template-library/blob/master/clocks.h
 ******************************************************************************/

#ifndef LIBS_COMMON_CLOCKS_HPP_
#define LIBS_COMMON_CLOCKS_HPP_

#include <msp430g2553.h>

#include <cstdint>

enum CLOCK_TYPE { CLOCK_TYPE_ACLK, CLOCK_TYPE_MCLK, CLOCK_TYPE_SMCLK, CLOCK_TYPE_ADCOSC };
  ...
  static void init(void) {
    switch (FREQUENCY) {
      ...
      case 12000000:
        ...
        DCOCTL = CALDCO_12MHZ;
        break;
      case 16000000: BCSCTL1 = CALBC1_16MHZ; DCOCTL = CALDCO_16MHZ;
      case 1000000:
        BCSCTL1 = CALBC1_1MHZ;
        DCOCTL = CALDCO_1MHZ;
        break;
...
struct MCLK_T {
  ...
  static void init(void) {
    switch (SOURCE::type) {
      case CLOCK_SOURCE_TYPE_DCO: break;
      case CLOCK_SOURCE_TYPE_VLO:
      case CLOCK_SOURCE_TYPE_LFXT1: BCSCTL2 |= SELM_3; break;
    }
  };
