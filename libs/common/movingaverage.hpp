 ...
 */
template<uint8_t numSamples, typename INPUT_TYPE = uint16_t, class SUM_TYPE = uint32_t>
...
public:
   ...
   */
  constexpr INPUT_TYPE filterNewSample(const INPUT_TYPE* const input) noexcept {
    ...
    filteredValue =  static_cast<INPUT_TYPE>(retVal);
